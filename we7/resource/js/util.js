...
util.url = function (action, querystring) {
    ...
    var url = siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&from=wxapp&';
...
function getSign(url, data, token) {
    ...
    if (sign || (data && data.sign)) {
        ...
        querystring = _.sortBy(querystring, 'name'); //去重
        ...
        token = token ? token : getApp().globalData.siteInfo.token;
...
util.request = function (option,auth=false) {
	// console.log("接口传递的值",option)
	// console.log("auth的值",auth)
	var url = siteInfo.url+option.url
	// console.log("打印",url)
	var data = option.data ? option.data : {}
	// console.log("传递的data值",option.data)
	...
	uni.request({
	    ...
	    success: function (response) {
	        ...
	        uni.hideLoading();
			// console.log("返回值",response.data.status)
	        if (response.data.status!=200) {
	            if (response.data.status == '102' || response.data.status == '101') {
					...
					util.message(response.data.msg, redirect, 'error');
	                return;
	            }
	        } else {
	    ...
	    fail: function (response) {
	        uni.hideNavigationBarLoading();
	        uni.hideLoading(); //如果请求失败，尝试从缓存中读取数据
...
util.requests = function (option) {
	console.log("接口传递的值",option)
	// console.log("全局",getApp().globalData.siteInfo)
	console.log("配置信息",siteInfo)
    var _ = require('./underscore.js');
...
util.message = function (title, redirect, type) {
    ...
    if (type == 'success') {
        ...
        uni.showModal({
            ...
            showCancel: false,
            complete: function () {
                if (title.indexOf('登录') >= 0 || title.indexOf('token') >= 0) {
                    uni.navigateTo({
                        url: '/gc_school/pages/login/login'
                    });
                }

                if (title.indexOf('先选择学校') >= 0) {
                    uni.navigateTo({
                        url: '/gc_school/pages/home/index'
                    });
                }

                if (title.indexOf('没有绑定商家') >= 0) {
                    uni.navigateTo({
                        url: '/gc_school/pages/user/index'
                    });
                }
...
util.user = util.getUserInfo; //封装微信等待提示，防止ajax过多时，show多次
